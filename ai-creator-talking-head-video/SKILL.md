...
10. Video capabilities must come from `assets/templates/model-config.example.json` or a user-supplied video config. Image generation follows the installed `imagegen` skill and built-in image_gen behavior. Do not hardcode video reference-image limits, lip-sync support, audio input support, or duration limits in user promises.
...
- `references/model-capabilities.md`: model capability fields, lip-sync routing, duration splitting, and payload mapping.
